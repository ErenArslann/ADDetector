...
$SARegex      = 'svc|service|sql|backup|scan|nps|adfs|krbtgt|_svc|\.svc'

# VPN / Remote Access / MFA group name patterns (case-insensitive regex)
$VPNRegex     = 'vpn|globalprotect|anyconnect|fortivpn|fortinet|forticlient|cisco.?vpn|sslvpn|alwayson.?vpn|directaccess|pulse.?secure|openvpn|wireguard|zscaler|netscaler.?gateway|paloalto'
$RARegex      = 'remote.?access|rd.?gateway|rdgateway|rds.?users|terminal.?services|citrix|xenapp|xendesktop|rdweb|workspace|tsgateway|jump.?server|bastion'
$MFARegex     = 'mfa|2fa|two.?factor|multi.?factor|duo|azure.?mfa|authenticator|rsa.?token|securid|yubikey|fido|passwordless|conditional.?access'

function Test-IsServiceAccount {
    param($r)
    if ($r.SPNCount -gt 0)                          { return $true }
    if ($r.SamAccountName -match $SARegex)           { return $true }
...
function Test-IsPrivileged {
    ...
    foreach ($g in $PrivGroups) {
        if ($r.MemberOfFlat -contains $g) { return $true }
    }
    return $false
}

function Get-MatchedGroups {
    param($MemberOfFlat, [string]$Pattern)
    if (-not $MemberOfFlat -or $MemberOfFlat.Count -eq 0) { return @() }
    return @($MemberOfFlat | Where-Object { $_ -match $Pattern })
}

function Test-HasVPNAccess  { param($r) return ((Get-MatchedGroups $r.MemberOfFlat $VPNRegex).Count -gt 0) }
function Test-HasMFAGroup   { param($r) return ((Get-MatchedGroups $r.MemberOfFlat $MFARegex).Count -gt 0) }
function Test-HasRemoteAcc  { param($r) return ((Get-MatchedGroups $r.MemberOfFlat $RARegex).Count  -gt 0) }
...
function New-Chk {
    ...
    $c.Text = $Text; $c.Font = $F.UISm
    $c.ForeColor = $C.FgSecondary
...
$mainSplit.Dock             = 'Fill'
$mainSplit.SplitterDistance = 980
$mainSplit.Panel1MinSize    = 620
$mainSplit.Panel2MinSize    = 290
$mainSplit.BackColor        = $C.Splitter
...
function New-UserRow {
    ...
    $r.VPNGroups       = Get-MatchedGroups $r.MemberOfFlat $VPNRegex
    $r.MFAGroups       = Get-MatchedGroups $r.MemberOfFlat $MFARegex
    $r.RAGroups        = Get-MatchedGroups $r.MemberOfFlat $RARegex
